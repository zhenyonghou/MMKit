...
/**
 *  MMSegmentedControlScrollStyle
 *  规定了两种不同的样式，一种灵感来自于QQ音乐（MMSegmentedControlScrollStyle未定义），一种灵感来自于美拍（MMSegmentedControlScrollStyle定义）.
 *  适用于不同场景，当segmented item id个数使得屏幕宽度能装下时，使用美拍式，否则，使用QQ音乐式更好。
 */
//#define MMSegmentedControlScrollStyle  1

typedef void (^IndexChangeBlock)(NSInteger index);

@interface MMSegmentedControl : UIControl
