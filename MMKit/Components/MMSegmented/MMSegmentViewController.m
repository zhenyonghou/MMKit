...
@implementation MMSegmentViewController
...
- (void)setupSegmentedControlWithPageTitles:(NSArray *)pageTitles
{
    if (!self.segmentedControl) {
        MMSegmentedControl *segmentedControl = [[MMSegmentedControl alloc] initWithFrame:CGRectMake(0,
                                                                                                    64,
...
- (void)segmentedSelectedPageShown:(NSInteger)index
{
    for (int k = 0; k < [self.pageControllers count]; ++k) {
        id<MMSegmentSubViewControllerProtocol> obj = self.pageControllers[k];
...
#ifndef MMSegmentedControlScrollStyle
...
#ifdef MMSegmentedControlScrollStyle
        CGFloat pageWidth = scrollView.frame.size.width;
