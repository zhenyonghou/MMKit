...
@interface MMStandardCameraManager ()

@end

@implementation MMStandardCameraManager

- (id)init
{
    if (self = [super init]) {
        _cameraOrientation = MMStandardCameraOrientationRear;
    }
...
- (void)showCameraInViewController:(UIViewController *)controller animated:(BOOL)animated
{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        if (!_pickerController) {
            ...
            if (_cameraOrientation == MMStandardCameraOrientationRear) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    if( picker.sourceType == UIImagePickerControllerSourceTypeCamera){
        ...
        __strong id<MMStandardCameraManagerDelegate> strongDelegate = self.delegate;
