...
@implementation MMDeviceID

@synthesize uid = _uid;

+ (NSString *)uniqueId {
    return [[[MMDeviceID alloc] initWithKey:@"deviceUID"] uniqueId];
}
 ...
 */
- (void)save {
    if (![MMDeviceID valueForUserDefaultsKey:_uidKey]) {
        [MMDeviceID setValue:_uid forUserDefaultsKey:_uidKey];
    }
    if (![MMDeviceID valueForKeychainKey:_uidKey service:_uidKey]) {
        [MMDeviceID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
